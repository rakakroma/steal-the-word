...
class HooliFloatingWordList extends LitElement {
    ...
    _wordList() {
        ...
        ${wordInPageList.map(wordObj => {
            ...
            <span class='word-span' @click="${()=>this._handleStartScrollToWord(wordObj)}">
            ${wordObj.word}           
             <span class='word-count' id="count-${wordObj.id}">${wordObj.countInCurrentPage}</span>
            </span>
            </h3>
    ...
    gotLookingWord(wordId, currentFocusCount){
        this.lookingWord = {...wordInPageList.find(wordObj=>wordObj.id === wordId), currentFocusCount}
        this.requestUpdate()
    }
    _handleRefresh() {
    ...
    _handleShowCount(wordId){
        this.renderRoot.getElementById(`count-${wordId}`).style.visibility = 'visible'
    }
      ...
      .draggable({
        ...
        listeners: {
          ...
          move(event) {
            ...
            position.x += event.dx
            position.y += event.dy
            document.querySelector('hooli-floating-word-list').style.transform =
