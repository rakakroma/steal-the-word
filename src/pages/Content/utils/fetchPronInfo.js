...
export const fetchPronInfo = async (targetWord, contextHere) => {
  ...
  if (kanjiLearningLangs.length === 1) {
    ...
    if (kanjiLearningLangs.includes('ja')) {
      const langDetectionResult = await languageDetectionForPronSearch(
        targetWord,
        contextHere
      );
  ...
  if (lang === 'ja') {
    ...
    const allLearningHakDialects = kanjiLearningLangs.filter((lang) =>
      lang.includes('hak')
    );
    ...
    pronounceDataResult = await fetch(moedictAPI(targetWord, 'h'))
      ...
      .then((data) => {
        ...
        const matchedResult = allGroupedProns
          .map((groupedProns, index) => {
            ...
            return (
              ...
              groupedProns
                .filter((pron) => {
                  return allLearningHakDialects.some((dialect) =>
                    pron.includes(abbrPair[dialect])
                  );
                })
                .join(', ')
                .toString()
          ...
          .join(', ')
          .toString();
        if (matchedResult.length === 1 && allLearningHakDialects.length === 1)
          return matchedResult.slice(2);
