    ...
    >
      ...
      <List sx={{ width: '100%' }}>
        {domainPageWords(contextList).map((arrayWithDomain) => {
          ...
          const imgUriInDB = domainInfo.icon
          ...
          return (
            <ListItem sx={{ flexDirection: 'column' }} key={arrayWithDomain[0]}>
              ...
              {arrayWithDomain[1].map((arrayWithUrl) => {
                const wordsFromThisPage = arrayWithUrl.words.reduce(
                  (acc, curr) => {
                    if (acc.some((wordObj) => wordObj.id === curr.wordId))
                      return acc;
                    const findWord = wordList.find(
